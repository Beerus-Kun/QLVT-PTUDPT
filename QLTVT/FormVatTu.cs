...
namespace QLTVT
{
    public partial class FormVatTu : Form
    {
        ...
        private void FormVatTu_Load(object sender, EventArgs e)
        {
            ...
            this.ctddTableAdapter.Connection.ConnectionString = Program.connstr;
            this.ctddTableAdapter.Fill(this.qLVTDataSet.CTDD);
        ...
        private bool kiemTraDuLieuDauVao()
        {
            ...
            txtDONVIVATTU.Text = txtDONVIVATTU.Text.Trim();
            ...
            {
                MessageBox.Show("Mã vật tư chỉ có chữ cái, số và dấu '-'", "Thông báo", MessageBoxButtons.OK);
                txtMAVT.Focus();
                return false;
            }

            if (txtMAVT.Text.Length > 20)
            {
            ...
            if (txtTENVT.Text.Length > 20)
            {
                MessageBox.Show("Tên vật tư không quá 20 kí tự", "Thông báo", MessageBoxButtons.OK);
                txtTENVT.Focus();
                return false;
            }
            if (txtDONVIVATTU.Text.Trim() == "")
            {
                MessageBox.Show("Không bỏ trống đơn vị tính", "Thông báo", MessageBoxButtons.OK);
                txtDONVIVATTU.Focus();
                return false;
            }
            if (txtDONVIVATTU.Text.Length > 6)
            {
                MessageBox.Show("Đơn vị vật tự không quá 6 kí tự", "Thông báo", MessageBoxButtons.OK);
                txtDONVIVATTU.Focus();
                return false;
            }
            if (Int32.Parse(txtTIEN.Text.Replace(",",""))<=0)
            {
        ...
        private void btnGHI_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            String tenVatTu = drv["TEN"].ToString();
            String donViTinh = drv["DONVI"].ToString();
            String soLuongTon = (drv["DONGIA"].ToString());
            String maLHH = (drv["MALHH"].ToString());

            /*Step 2*/
            int viTriConTro = bdsVatTu.Position;
            int viTriMaVatTu = bdsVatTu.Find("MAHH", txtMAVT.Text);
           
            if (viTriMaVatTu >= 0 && viTriConTro != viTriMaVatTu)
            {
                MessageBox.Show("Mã hàng hóa đã tồn tại!", "Thông báo", MessageBoxButtons.OK);
                return;
            }
            else
            {
                ...
                if (dr == DialogResult.OK)
                {
                    try
                    {
                        ...
                        this.bdsVatTu.EndEdit();
                        this.hangHoaTableAdapter.Update(this.qLVTDataSet.HANGHOA);
        ...
        private void btnXOA_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            if (bdsCTHD.Count > 0)
            {
                MessageBox.Show("Không thể xóa vật tư này vì đã lập hóa đơn", "Thông báo", MessageBoxButtons.OK);
                return;
            }
            ...
            * để nếu chẳng may người dùng ấn hoàn tác thì quất luôn*/
            ...
            string cauTruyVanHoanTac =
            "INSERT INTO DBO.HANGHOA( MAHH,TEN,DONVI,MALHH,DONGIA) " +
            " VALUES( '" + drv["MAHH"].ToString() + "','" +
                        drv["TEN"].ToString() + "','" +
                        drv["DONVI"].ToString() + "', '" +
                        drv["MALHH"].ToString() + "', " +
                        drv["DONGIA"].ToString() + " ) ";
